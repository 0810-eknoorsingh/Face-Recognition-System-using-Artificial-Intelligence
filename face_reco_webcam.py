...
video_capture = cv2.VideoCapture(0) #capturing the video from webcam

eknoor_image = fr.load_image_file('image1.jpg') 
#load the picture 
eknoor_face_encoding = fr.face_encodings(eknoor_image)[0] #analyse the picture and do encoding of the face and will return array for number of faces

known_face_encondings = [eknoor_face_encoding] #array for known faces
known_face_names = ["Eknoor"] #name displayed when face is detected
...
while True: #created the loop to take all the frameworks of camera
    ...
    for (top, right, bottom, left), face_encoding in zip(face_locations, face_encodings): #iterate over all the locations and encodings
        ...
        cv2.rectangle(frame, (left, top), (right, bottom), (0, 0, 255), 2)#customizing the frame which is going tobe formed on the face 

        cv2.rectangle(frame, (left, bottom -35), (right, bottom), (0, 0, 255), cv2.FILLED) #will tell the name of the person if known otherwise unknown
        font = cv2.FONT_HERSHEY_SIMPLEX #font for the text 
        cv2.putText(frame, name, (left + 6, bottom - 6), font, 1.0, (255, 255, 255), 1)# text which is written below the frame
...
cv2.destroyAllWindows() #destroy everything after we quit q